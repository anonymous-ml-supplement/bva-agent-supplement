...
IN_FILES = [
    ...
    "../artifacts/reported_results/joint_calibration/joint_calibration_missing_outcomes_raw.csv",
]

OUT = "../artifacts/reported_results/prm_baseline/prm_cal_missing_outcomes_runlist.csv"
...
cal_seeds = list(range(20, 120))

jobs = []
for s in cal_seeds:
    if 1 not in pool[s]:
        jobs.append({
            "episode_id": f"revision_prm_cal_missing_verified_s{s}",
            "case": "dependency_sensitive",
            "policy": "always_verify",
            "seed": str(s),
            "notes": "PRM baseline missing verified outcome for calibration split.",
            "status": "",
        })
    if 0 not in pool[s]:
        jobs.append({
            "episode_id": f"revision_prm_cal_missing_unverified_s{s}",
            "case": "dependency_sensitive",
            "policy": "no_verify",
            "seed": str(s),
            "notes": "PRM baseline missing unverified outcome for calibration split.",
            "status": "",
        })
